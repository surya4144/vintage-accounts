...
export default function App() {
  ...
  const totalOnlineExpenses = onlineExpenses.reduce(
  ...
  const totalOnlineBalance =
    yesterdayOnline + Number(onlineSale) - totalOnlineExpenses;
  const totalAmountLeft = totalCashInHand + totalOnlineBalance;

  // --- HANDLERS ---
  const addOnlineExpense = () =>
    setOnlineExpenses([
      ...onlineExpenses,
  ...
  const addCashExpense = () =>
    setCashExpenses([
      ...cashExpenses,
      { id: Date.now(), name: '', amount: 0, type: 'Cash' },
    ]);
  ...
  return (
    ...
    >
      <h1 style={{ textAlign: 'center', color: '#1f2937' }}>
        Vintage Restaurant - Daily Accounts
      </h1>
        ...
        <div style={flexRow}>
          ...
          <label style={{ flex: 1 }}>
            ...
            <input
              type="number"
              value={yesterdayCash}
              onChange={(e) => setYesterdayCash(Number(e.target.value))}
              style={inputStyle}
            />
          </label>
          <label style={{ flex: 1 }}>
            ...
            <input
              type="number"
              value={yesterdayOnline}
              onChange={(e) => setYesterdayOnline(Number(e.target.value))}
              style={inputStyle}
            />
          </label>
        </div>
        ...
        <div style={flexRow}>
          <label style={{ flex: 1 }}>
            ...
            <input
              type="number"
              value={cashSale}
              onChange={(e) => setCashSale(Number(e.target.value))}
              style={inputStyle}
            />
          </label>
          <label style={{ flex: 1 }}>
            ...
            <input
              type="number"
              value={onlineSale}
              onChange={(e) => setOnlineSale(Number(e.target.value))}
              style={inputStyle}
            />
          </label>
          <div style={{ ...totalBoxStyle, flex: 1 }}>
            Total Sale: <br />
      ...
      <div style={{ display: 'flex', gap: '20px', flexWrap: 'wrap' }}>
          ...
          {onlineExpenses.map((exp) => (
            ...
            >
              <input
                ...
                value={exp.name}
                onChange={(e) =>
                  updateOnlineExpense(exp.id, 'name', e.target.value)
                }
                style={inputStyle}
              />
              <input
                ...
                value={exp.amount}
                onChange={(e) =>
                  updateOnlineExpense(exp.id, 'amount', e.target.value)
                }
                style={inputStyle}
              />
            </div>
          ))}
          <button onClick={addOnlineExpense} style={btnStyle}>
            + Add Online Expense
          </button>
          <p style={{ marginTop: '15px' }}>
            Total Online Expenses: <strong>{totalOnlineExpenses}</strong>
          </p>
        </div>
        ...
        <div style={{ ...cardStyle, flex: 1, minWidth: '300px' }}>
          ...
          {cashExpenses.map((exp) => (
            ...
            >
              <input
                ...
                value={exp.name}
                onChange={(e) =>
                  updateCashExpense(exp.id, 'name', e.target.value)
                }
                style={inputStyle}
              />
              <input
                ...
                value={exp.amount}
                onChange={(e) =>
                  updateCashExpense(exp.id, 'amount', e.target.value)
                }
                style={inputStyle}
              />
              <select
                value={exp.type}
                onChange={(e) =>
                  updateCashExpense(exp.id, 'type', e.target.value)
                }
                style={inputStyle}
              >
          ...
          <button onClick={addCashExpense} style={btnStyle}>
            + Add Cash/Credit Expense
          </button>
          <p style={{ marginTop: '15px', marginBottom: '5px' }}>
            Total Cash Expenses: <strong>{totalCashExpenses}</strong>
          </p>
          <p style={{ fontSize: '0.85em', color: '#6b7280', margin: 0 }}>
            (Credit Logged: {totalCreditExpenses})
          </p>
        </div>
      </div>
      ...
      <div style={{ ...cardStyle, backgroundColor: '#e5e7eb' }}>
        ...
        <div style={flexRow}>
          ...
          <h3 style={{ borderBottom: '2px solid black', flex: 1 }}>
            Total Left: <br />
            {totalAmountLeft}
          </h3>
        </div>
        <button
          onClick={saveDailyAccounts}
          style={{
            ...btnStyle,
